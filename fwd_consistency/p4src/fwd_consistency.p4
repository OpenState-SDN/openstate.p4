...
field_list_calculation port_select {
    input {
        random_port;
    }
    algorithm : crc16;
...
action forward(eth_dst,ip_dst,tcp_dst) {
    ...
    //tcpLength is the length in bytes of header tcp + payload tcp
...
action forward_back(eth_src,ip_src,tcp_src) {
    ...
    //As stated in the forward action, we need to calculate tcpLength 
    modify_field(routing_metadata.ipv4Length, ipv4.ihl);
    add_to_field(routing_metadata.ipv4Length, ipv4.ihl);
    add_to_field(routing_metadata.ipv4Length, ipv4.ihl);
    add_to_field(routing_metadata.ipv4Length, ipv4.ihl);
